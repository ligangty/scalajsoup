...
import TokeniserState._

private[parser] sealed trait TokeniserState {
...
private[parser] object TokeniserState {
  ...
  private[parser] val attributeSingleValueCharsSorted: Array[Char] = Array[Char]('\'', '&', nullChar).sorted
  private[parser] val attributeDoubleValueCharsSorted: Array[Char] = Array[Char]('"', '&', nullChar).sorted
  private[parser] val attributeNameCharsSorted: Array[Char] = Array[Char]('\t', '\n', '\r', '\f', ' ', '/', '=', '>', nullChar, '"', '\'', '<').sorted
  private[parser] val replacementChar: Char = Tokeniser.replacementChar
  private[parser] val replacementStr: String = String.valueOf(Tokeniser.replacementChar)
  private[parser] val eof: Char = CharacterReader.EOF
   ...
   */
  private[parser] def handleDataEndTag(t: Tokeniser, r: CharacterReader, elseTransition: TokeniserState) {
    if (r.matchesLetter) {
      val name: String = r.consumeLetterSequence
      t.tagPending.appendTagName(name.toLowerCase)
      t.dataBuffer.append(name)
  ...
  private[parser] def handleDataDoubleEscapeTag(t: Tokeniser, r: CharacterReader, primary: TokeniserState, fallback: TokeniserState) {
    if (r.matchesLetter) {
      val name: String = r.consumeLetterSequence
