 ...
 * <tr><td><code>:not(<em>selector</em>)</code></td><td>elements that do not match the <em>selector</em>. See also [[Elements.not(String)]]</td><td><code>div:not(.logo)</code> finds all divs that do not have the "logo" class.<br /><code>div:not(:has(div))</code> finds divs that do not contain divs.</td></tr>
...
object Selector {
  ...
  private[select] def filterOut(elements: Traversable[Element], outs: Traversable[Element]): Elements = {
    val output: Elements = new Elements
    for (el <- elements) {
